...
void Processor::set_warmup_insts(const ulong warmup_insts) {
    for (unsigned int i = 0; i < cores.size(); i++) {
        cores[i]->set_warmup_insts(warmup_insts);
    }
}

bool Processor::is_warmup_complete() {
    for (unsigned int i = 0; i < cores.size(); i++) {
        if(!cores[i]->is_warmup_complete())
          return false;
    }
...
void Core::tick()
{
    if(is_in_warmup() && is_warmup_complete())
      return; // do not tick() further if the warmup instruction count is reached
...
void Core::set_warmup_insts(const ulong _warmup_insts) {
    warmup_insts = _warmup_insts;
}

bool Core::is_warmup_complete() {
    return warmup_insts <= cpu_inst.value();
}

bool Core::is_in_warmup() {
  return warmup_insts > 0;
}
