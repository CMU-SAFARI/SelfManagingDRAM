...
Refresh<DSARP>::Refresh(Controller<DSARP>* ctrl) : ctrl(ctrl) {
  ...
  max_bg_count = ctrl->channel->spec->org_entry.count[(int)DSARP::Level::BankGroup];
  max_bank_count = ctrl->channel->spec->org_entry.count[(int)DSARP::Level::Bank];
  max_bank_bg_count = max_bank_count * max_bg_count;
  ...
  for (int r = 0; r < max_rank_count; r++) {
    ...
    bank_refresh_backlog.push_back(new vector<int>(max_bank_bg_count, 0));
    // TODO: this is a bug in original DSARP implementation???
    vector<int> sa_counters(max_bank_bg_count, 0);
    subarray_ref_counters.push_back(sa_counters);
  }

  level_chan  = (int)DSARP::Level::Channel;
  level_rank  = (int)DSARP::Level::Rank;
  level_bg    = (int)DSARP::Level::BankGroup;
...
void Refresh<DSARP>::early_inject_refresh() {
  ...
  vector<bool> is_bank_occupied(max_rank_count * max_bank_bg_count, false);
  ...
  for (auto req: rdq.q)
  {
    ...
    int ridx = req.addr_vec[level_rank] * max_bank_bg_count;
    int bidx = req.addr_vec[level_bg] * max_bank_count + req.addr_vec[level_bank];
    is_bank_occupied[ridx+bidx] = true;
    //printf("Bank %d occuppied\n", bidx);
  }

  //printf("REFQ size %d -- RDQ size %d\n", ctrl->otherq.size(), ctrl->readq.size());

  // Try to pick an idle bank to refresh per rank
  for (int r = 0; r < max_rank_count; r++) {
    // Randomly pick a bank to examine
    int bidx_start = rand_r(&rand_seed) % max_bank_bg_count;

    for (int b = 0; b < max_bank_bg_count; b++)
    {
      int bidx = (bidx_start + b) % max_bank_bg_count;
      // Idle cycle only
      if (is_bank_occupied[(r * max_bank_bg_count) + bidx])
        continue;
      ...
      for (Request req : ctrl->otherq.q)
        if (req.type == Request::Type::REFRESH
            ...
            && req.addr_vec[level_rank] == r 
            && (req.addr_vec[level_bg] * max_bank_count + req.addr_vec[level_bank]) == bidx)
          pending_ref = true;
      if (pending_ref)
        continue;
      ...
      // Refresh now
      //printf("Early Inject Refresh\n");
      refresh_target(ctrl, r, bidx/4, bidx%4, subarray_ref_counters[r][bidx]);
...
void Refresh<DSARP>::inject_refresh(bool b_ref_rank) {
  ...
  // Bank-level refresh. Simultaneously issue to all ranks (better performance than staggered refreshes).
    ...
    for (auto rank : ctrl->channel->children) {
      ...
      // Next time, refresh the next bank in the same rank
      bank_ref_counters[rid] = (bank_ref_counters[rid] + 1) % max_bank_bg_count;
      ...
      refresh_target(ctrl, rid, bid/4, bid%4, subarray_ref_counters[rid][bid]);
      // Get 1 ref credit
      (*(bank_refresh_backlog[rid]))[bid]++;
  ...
  for (int ref_rid = 0; ref_rid < max_rank_count; ref_rid++)
  {
    ...
    for (auto req : ctrl->writeq.q) {
      if (req.addr_vec[level_rank] == ref_rid) {
        sorted_bank_demand[req.addr_vec[level_bg] * max_bank_count + req.addr_vec[level_bank]].first++;
        total_wr++;
      }
    }
    ...
    int top_idle_idx = 0;
    for (int i = 0; i < max_bank_bg_count; i++) {
    ...
    int ref_bid_idx = (top_idle_idx == 0) ? 0 : rand_r(&rand_seed) % top_idle_idx;
    ...
    if ((*(bank_refresh_backlog[ref_rid]))[ref_bid] < backlog_max
        && ctrl->otherq.q.size() < ctrl->otherq.max) {
      refresh_target(ctrl, ref_rid, ref_bid/4, ref_bid%4, subarray_ref_counters[ref_rid][ref_bid]);
      // Get 1 ref credit
...
void Refresh<DSARP>::tick_ref() {
  ...
  //printf("refi: %d rfc: %d\n", refresh_interval, ctrl->channel->spec->speed_entry.nRFCpb);
  ...
  // printf("CLK: %ld REFRESHED: %ld DIFF: %ld\n", clk, refreshed, clk-refreshed);
