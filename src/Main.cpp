...
void run_cputrace(Config& configs, Memory<T, Controller>& memory, const std::vector<std::string>& files)
{
    ...
    // bool is_warming_up = (warmup_insts != 0); // REMOVE

    // configure processor warmup
    assert(warmup_insts >= 0 && "ERROR: warmup_insts cannot be a negative decimal number.");
    proc.set_warmup_insts((ulong)warmup_insts);

    auto start = std::chrono::steady_clock::now();

    for (long i = 0; ; i++) {
        if (proc.is_warmup_complete())
            break;
    ...
    // TODO: it is not appropriate to keep warming up cores that have already completes the warmup_insts count.
    // This leads to executing different portions of the traces when simulating multi-programmed workloads
    // The correct behavior would be to stop ticking a core once it reaches the warmup_insts count.
    ...
    proc.set_warmup_insts(0); // exit from the warmup phase
    ...
    mem_tick = configs.get_int("mem_tick"); // reload option in case it is different in sim_options. TODO: think of a nicer way of doing this
    memory.reload_options(configs); // TODO: think of a nicer way of doing this
    ...
    for (long i = 0; ; i++) {
        if (((i % tick_mult) % mem_tick) == 0) { // Hasan: we use mem_tick to check when to tick the CPU. 
...
void start_run(Config& configs, T* spec, const vector<std::string>& files) {
    ...
    spec->set_subarray_size(configs.get_uint("subarray_size"));
...
{
    if (argc < 2) {
        printf("Usage: %s <configs-file> --mode=cpu,dram [--stats <filename>] -t <trace-filename1> -t <trace-filename2> [-p key=value] [-c key=value]\n"
            "Example: %s ramulator-configs.cfg --mode=cpu -t cpu.trace -p warmup_insts=0 -c translation=Random\n", argv[0], argv[0]);
        printf("-p parameters apply to the whole simulation while -c parameters are applied after warmup.\n");
        return 0;
    }
...
//    }
    // Various refresh mechanisms
      ...
      DSARP* dsddr3_dsarp = new DSARP(configs["org"], configs["speed"], DSARP::Type::DSARP, configs.get_int("subarray_size"));
