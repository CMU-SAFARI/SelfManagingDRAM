...
namespace ramulator {
    ...
    public:
        ...
        bool access(const vector<int>& addr_vec, const bool move_to_LRU = false) {
            ...
            if(is_hit(addr_vec)) {
                ...
                //if(!move_to_LRU)
                //    printf("CROWTable: Promoting crow LRU state: bank: %d, SA_id: %lu, row %lu \n", 
                //        addr_vec[int(T::Level::Bank)], cur_entry->row_addr/SA_size,
                //        cur_entry->row_addr);
                //else
                //    printf("CROWTable: Demoting crow LRU state: bank: %d, SA_id: %lu, row %lu \n",
                //        addr_vec[int(T::Level::Bank)], cur_entry->row_addr/SA_size,
                //        cur_entry->row_addr);
		...
		CROWEntry* add_entry(const vector<int>& addr_vec, const bool FR) {
            ...
            //printf("CROWTable: Adding entry. lru_ind: %d \n", lru_ind);
            ...
            if (freeLoc == -1) {
		        //cur_entry = get_entry(addr_vec, get_next_copy_row_id(addr_vec));
		        cur_entry = get_LRU_entry(addr_vec);
                
                // int discarded_row_id = cur_entry->row_addr;
                // printf("CROWTable: Removing LRU crow. bank: %d, SA_id: %d, row: %d \n", 
                //         addr_vec[int(T::Level::Bank)], discarded_row_id/SA_size, discarded_row_id);

                // printf("CROWTable: Opening space for: bank: %d, SA_id: %d, row: %d \n", 
                //         addr_vec[int(T::Level::Bank)], addr_vec[int(T::Level::Row)]/SA_size, addr_vec[int(T::Level::Row)]);
            ...
            //printf("CROWTable:: Promoting crow LRU status: bank: %d, SA_id: %d, row: %d \n", 
            //        addr_vec[int(T::Level::Bank)], row_id/SA_size, row_id);
        ...
        //CROWEntry* get_entry(const vector<int>& addr_vec) { 
        //    int offset = calc_entries_offset(addr_vec);
        //    return &entries[offset + get_hit_location(addr_vec)];
        //}
        ...
        CROWEntry* get_hit_entry(const vector<int>& addr_vec){
	        ...
	        for (uint i = 0; i < num_copy_rows; i++) {
            	if (entries[offset + i].row_addr == (ulong)row_addr && 
    ...
    private:
        ...
        //int get_hit_location(const vector<int>& addr_vec) {
        //    int offset = calc_entries_offset(addr_vec);

        //    int row_addr = addr_vec[int(T::Level::Row)];
	    //    for (uint i = 0; i < num_copy_rows; i++) {
        //    	if (entries[offset + i].row_addr == row_addr && 
	    //    			entries[offset + i].valid)
        //        	return (int) i;
        //    }
        ...
        void do_weak_row_mapping() {
            ...
            for(int r = 0; r < spec->org_entry.count[int(T::Level::Rank)]; r++){
                ...
                for(int b = 0; b < spec->org_entry.count[int(T::Level::Bank)]; b++) {
                    ...
                    for(uint32_t i = 0; i < num_SAs; i++) {
                        addr_vec[int(T::Level::Row)] = i*SA_size;
                        int offset = calc_entries_offset(addr_vec);
                        for(uint32_t j = 0; j < num_weak_rows; j++) {
