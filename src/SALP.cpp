...
SALP::SALP(Org org, Speed speed, Type type, uint32_t sa_size) :
    type(type),
    //n_sa(n_sa),
...
SALP::SALP(const string& org_str, const string& speed_str, const string& type_str, uint32_t sa_size) :
    SALP(org_map[org_str], speed_map[speed_str], type_map[type_str], sa_size)
{
}

SALP::SALP(const Config& configs, const string& type_str) : SALP(configs["org"], 
        configs["speed"], type_str, configs.get_uint("subarray_size")){
...
void SALP::set_subarray_size(uint32_t sa_size) {
    _sa_size = sa_size;
