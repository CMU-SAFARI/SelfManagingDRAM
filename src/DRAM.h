...
public:
    ...
    void update_num_subarrays(const uint32_t num_sa); // Hasan
    ...
    // SMD
    // bool check_only_this_level(const typename T::Command cmd, const long clk) const;
    void init_REFpb_history();
    int get_open_SA(const uint32_t rank_id, const uint32_t bank_id) const;
    int get_open_SA(const std::vector<int>& addr_vec) const;
    // SMD -- END
    ...
    void close_row(const std::vector<int>& addr_vec, const long clk); // Hasan
    void mark_partial_act_nack(const std::vector<int>& addr_vec, const long clk); // Hasan
    bool is_partially_active(const std::vector<int>& addr_vec); // Hasan
    // Update statistics:
    ...
    int cycles_since_last_act(const vector<int>& addr_vec, long clk) {
        ...
        //} else
        //    return children[addr_vec[int(level) + 1]]->cycles_since_last_act(addr_vec, clk);
    ...
    int cycles_since_last_act(const int* addr, long clk) {
        if(level == T::Level::Bank || !children.size()){
            assert(prev[int(T::Command::ACT)].size() == 1);
            return (clk - prev[int(T::Command::ACT)][0]);
        } else
    ...
    int cycles_since_last_act(long clk) { 
        // Should be called only from the bank level
        assert(level == T::Level::Bank || !children.size());
    ...
    spec(spec), level(level), id(0), parent(NULL)
{
    ...
    // printf("Recursively construct %d nodes at level %d\n", child_max, level);
...
template <typename T>
void DRAM<T>::update_num_subarrays(const uint32_t num_sa) {
...
// template <typename T>
// bool DRAM<T>::check_only_this_level(const typename T::Command cmd, const long clk) const {
//     return next[int(cmd)] == -1 || clk >= next[int(cmd)];
// }
...
template <typename T>
void DRAM<T>::close_row(const std::vector<int>& addr_vec, const long clk) {
    update(T::Command::ACT_NACK, addr_vec.data(), clk);
}

template <typename T>
void DRAM<T>::mark_partial_act_nack(const std::vector<int>& addr_vec, const long clk) {
    update(T::Command::ACT_PARTIAL_NACK, addr_vec.data(), clk);
}

template <typename T>
bool DRAM<T>::is_partially_active(const std::vector<int>& addr_vec) {
    
    if (level != T::Level::Bank)
        return children[addr_vec[int(level)+1]]->is_partially_active(addr_vec);

    return state == T::State::PartiallyOpened;
}
...
void DRAM<T>::update_timing(typename T::Command cmd, const int* addr, long clk)
{
    ...
    if (id != addr[int(level)]) {
        for (auto& t : timing[int(cmd)]) {
            ...
            assert (t.dist == 1);
            //if (cmd == T::Command::REFPB)
                //printf("Sibling -- From REFPB to CMD: %d Timing: %d Future: %ld Next: %ld at level: %d\n", t.cmd, t.val, clk+t.val, next[int(t.cmd)], int(level));
    ...
    for (auto& t : timing[int(cmd)]) {
        ...
        //if (cmd == T::Command::REFPB)
            //printf("From REFPB to CMD: %d Timing: %d Future: %ld Next: %ld at level: %d\n", t.cmd, t.val, clk+t.val, next[int(t.cmd)], int(level));
...
template <typename T>
void DRAM<T>::init_REFpb_history() {
    assert(level == T::Level::Channel);

    for (auto& rank : this->children)
        for (auto& bg : rank->children)
            for (auto& bank : bg->children)
                bank->prev[int(T::Command::REF)].resize(1, -1); // initialize history
}

template <typename T>
int DRAM<T>::get_open_SA(const uint32_t rank_id, const uint32_t bank_id) const {

    // make sure this is called from Channel level
    assert(level == T::Level::Channel);
    
    uint bgid = bank_id / this->spec->org_entry.count[int(T::Level::Bank)];
    uint bid = bank_id % this->spec->org_entry.count[int(T::Level::Bank)];
    
    auto bank_ptr = children[rank_id]->children[bgid]->children[bid];

    assert(bank_ptr->row_state.size() <= 1);

    if (bank_ptr->row_state.size() == 1)
        return bank_ptr->row_state.begin()->first;

    return -1; // no open subarray in the bank
}

template <typename T>
int DRAM<T>::get_open_SA(const std::vector<int>& addr_vec) const {

    // make sure this is called from Channel level
    assert(level == T::Level::Channel);
    
    uint global_bank_id = addr_vec[int(T::Level::BankGroup)]*this->spec->org_entry.count[int(T::Level::Bank)] + addr_vec[int(T::Level::Bank)];
    return get_open_SA(addr_vec[int(T::Level::Rank)], global_bank_id);
}
